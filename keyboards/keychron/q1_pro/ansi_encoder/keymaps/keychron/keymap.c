...
enum custom_keycodes {
    KC_GLOBE_KEY = NEW_SAFE_RANGE,
};

#define FN_WIN MO(WIN_FN)

static bool globe_pressed;
static bool fn_combo_active;
    ...
    switch (keycode) {
        case KC_GLOBE_KEY:
            if (record->event.pressed) {
                ...
                globe_pressed = false;
                if (fn_combo_active) {
                    fn_combo_active = false;
                    uint8_t fn_layer = get_highest_layer(default_layer_state) == MAC_BASE ? MAC_FN : WIN_FN;
                    layer_off(fn_layer);
                } else {
            ...
            return false;
        case KC_RCTL:
            if (record->event.pressed && globe_pressed && !fn_combo_active) {
                unregister_code(KC_GLOBE);
                fn_combo_active = true;
                uint8_t fn_layer = get_highest_layer(default_layer_state) == MAC_BASE ? MAC_FN : WIN_FN;
                layer_on(fn_layer);
                return false;
            }
            if (!record->event.pressed && fn_combo_active) {
                fn_combo_active = false;
                uint8_t fn_layer = get_highest_layer(default_layer_state) == MAC_BASE ? MAC_FN : WIN_FN;
                layer_off(fn_layer);
                return false;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_ansi_82(
        ...
        KC_LCTL,  KC_LOPTN, KC_LCMMD,                               KC_SPC,                                 KC_LCMMD, KC_GLOBE_KEY, KC_RCTL, KC_LEFT, KC_DOWN, KC_RGHT),
